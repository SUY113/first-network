...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-tools:$IMAGETAG_CLI peer version | sed -ne 's/ Version: //p' | head -1)
...
function networkUp() {
  ...
  IMAGE_TAG=$IMAGETAG IMAGE_TAG_CLI=$IMAGETAG_CLI docker-compose ${COMPOSE_FILES} up -d 2>&1
  docker ps -a
  if [ $? -ne 0 ]; then
...
function networkDown() {
  # stop org3 containers also in addition to orgAccountant and orgStaff, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/first-network --rm hyperledger/fabric-tools:$IMAGETAG_CLI rm -Rf /tmp/first-network/ledgers-backup
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for OrgAccountant and OrgStaff"
  ./ccp-generate.sh
}
...
# and two Peer Orgs (``OrgAccountant`` & ``OrgStaff``) each managing and maintaining two peer nodes.
...
# (``peer0.orgAccountant.example.com`` & ``peer0.orgStaff.example.com``).  Secondly, we point to
...
while getopts "h?c:t:d:f:s:l:i:o:anv" opt; do
  ...
  i)
    ...
    IMAGETAG_CLI=$(go env GOARCH)"-"$OPTARG
    ;;
  o)
